...
(function (root, factory) {
  ...
  var TAXONOMY =
    (typeof globalThis !== "undefined" && globalThis.ESBvaktinTaxonomy) || {};
  var utils =
    (typeof globalThis !== "undefined" && globalThis.ESBvaktinTrackerUtils) ||
    {};

  var VERDICT_LABELS =
    (TAXONOMY.verdictLabels && TAXONOMY.verdictLabels.factual) || {};
  var EPISTEMIC_TYPE_LABELS = TAXONOMY.epistemicTypeLabels || {};
  var VERDICT_DESCRIPTIONS =
    (TAXONOMY.verdictDescriptions && TAXONOMY.verdictDescriptions.factual) ||
    TAXONOMY.verdictDescriptions ||
    {};
  ...
  var findReportForSource =
    utils.findReportForSource ||
    function () {
      return null;
    };
  var buildReturnUrl =
    utils.buildReturnUrl ||
    function () {
      return "";
    };
  var withReturnUrl =
  ...
  function confidenceLabel(pct) {
    if (pct >= 85) return "Mikil vissa";
    if (pct >= 70) return "Nokkur vissa";
    if (pct >= 50) return "Takmörkuð vissa";
    return "Mikil óvissa";
  }

  var EVIDENCE_ID_RE = /\b([A-Z]+-[A-Z]+-\d+)\b/g;
  function linkifyEvidenceIds(html) {
    return html.replace(EVIDENCE_ID_RE, function (_, id) {
      return (
        '<a href="/heimildir/' +
        id.toLowerCase() +
        '/" class="evidence-link" data-evidence-id="' +
        id +
        '">' +
        id +
        "</a>"
  ...
  function renderClaimCard(claim, opts) {
    ...
    if (epistemicType !== "factual" && EPISTEMIC_TYPE_LABELS[epistemicType]) {
      ...
      epistemicBadgeHtml =
        '<span class="ct-epistemic-badge epistemic-' +
        epistemicType +
        '"' +
        (epistemicTooltip
          ? ' title="' + escapeHtml(epistemicTooltip) + '"'
          : "") +
        ">" +
        escapeHtml(EPISTEMIC_TYPE_LABELS[epistemicType]) +
        "</span>";
    }
    ...
    var EPISTEMIC_CONTEXT = {
      prediction:
        "Þetta er spá — mat byggir á samstöðu heimilda og gæðum rökfærslu.",
      counterfactual: "Þetta er tilgáta um atburði sem gerðust ekki.",
      hearsay:
        "Þetta byggir á ónafngreindum heimildum sem ekki er hægt að staðfesta.",
    };

    var detailsHtml = "";
    if (epistemicType === "hearsay") {
      detailsHtml +=
        '<div class="ct-hearsay-warning">' +
      ...
      EPISTEMIC_CONTEXT[epistemicType]
    ) {
      var contextLabel = epistemicType === "prediction" ? "ℹ Spá" : "ℹ Tilgáta";
      detailsHtml +=
        '<div class="ct-epistemic-context epistemic-' +
        epistemicType +
        '">' +
        "<strong>" +
        contextLabel +
        "</strong> — " +
        escapeHtml(EPISTEMIC_CONTEXT[epistemicType]) +
        "</div>";
    }
    if (claim.explanation_is) {
      detailsHtml +=
        '<div class="ct-detail"><strong>Útskýring:</strong> ' +
        linkifyEvidenceIds(escapeHtml(claim.explanation_is)) +
        "</div>";
    }
    if (claim.missing_context_is) {
      detailsHtml +=
        '<div class="ct-detail"><strong>Samhengi sem vantar:</strong> ' +
        linkifyEvidenceIds(escapeHtml(claim.missing_context_is)) +
        "</div>";
    }
    if (claim.canonical_text_en) {
      detailsHtml +=
        '<div class="ct-detail ct-english"><strong>English:</strong> ' +
        escapeHtml(claim.canonical_text_en) +
        "</div>";
    }
    ...
    if (supportingEvidence.length > 0 || contradictingEvidence.length > 0) {
      ...
      if (supportingEvidence.length > 0) {
        evidenceHtml +=
          '<div class="ct-evidence-group"><strong>Heimildir:</strong> ' +
          renderEvidenceLinks(supportingEvidence) +
          "</div>";
      }
      if (contradictingEvidence.length > 0) {
        evidenceHtml +=
          '<div class="ct-evidence-group ct-evidence-contra"><strong>Andstæðar heimildir:</strong> ' +
          renderEvidenceLinks(contradictingEvidence) +
          "</div>";
      }
      evidenceHtml += "</div>";
    ...
    if (claim.sightings && claim.sightings.length) {
      ...
      var sightingItems = claim.sightings
        .map(function (sighting) {
          var typeLabel =
            SOURCE_TYPE_LABELS[sighting.source_type] ||
          ...
          var matchedReport = reportLookup
            ? findReportForSource(sighting, reportLookup)
            : null;
          var internalHref =
            matchedReport && matchedReport.slug
              ? withReturnUrl(
                  "/umraedan/" + matchedReport.slug + "/",
                  buildReturnUrl(cardId),
                )
              : "";
          var href = internalHref || sighting.source_url || "";
          var externalAttrs = internalHref
            ? ""
      ...
      detailsHtml +=
        ...
        '<span class="ct-sightings-label">Birtist í ' +
        sightingCount +
        " umræðu" +
        (sightingCount > 1 ? "m" : "") +
        "</span>" +
    ...
    var sightingBadge =
      claim.sighting_count > 0
        ? '<span class="ct-sighting-count" title="Fjöldi tilvitana">' +
    ...
    return (
      '<div class="ct-card' +
      (isFocused ? " ct-card--focused" : "") +
      '" id="' +
      escapeHtml(cardId) +
      '" data-slug="' +
      escapeHtml(claim.claim_slug) +
      '">' +
      '<div class="ct-card-header" role="button" tabindex="0" aria-expanded="false">' +
      '<div class="ct-card-main">' +
      '<span class="ct-verdict-pill ' +
      verdictClass +
      '" title="' +
      escapeHtml(
        TAXONOMY.verdictDescription
          ? TAXONOMY.verdictDescription(claim.verdict, epistemicType)
          : VERDICT_DESCRIPTIONS[claim.verdict] || "",
      ) +
      '">' +
      verdictLabel +
      "</span>" +
      epistemicBadgeHtml +
      '<span class="ct-category-tag">' +
      categoryLabel +
      "</span>" +
      sightingBadge +
      maturityBadge +
      ...
      '<span class="ct-confidence" title="' +
      confidenceLabel(confidencePct) +
      " (" +
      confidencePct +
      '%)">' +
      '<span class="ct-confidence-bar" style="width: ' +
      confidencePct +
      '%"></span>' +
      '<span class="ct-confidence-label">' +
      confidenceLabel(confidencePct) +
      "</span>" +
      "</span>" +
