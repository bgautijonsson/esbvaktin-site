...
function resolveRecords(type, values) {
  const content = loadContentData();

  switch (type) {
    case "reports":
      return resolveLinkedItems(values, content.lookups.reportsBySlug, buildReportCard);
    case "claims":
      return resolveLinkedItems(values, content.lookups.claimsBySlug, buildClaimCard);
    case "entities":
      return resolveLinkedItems(values, content.lookups.entitiesBySlug, buildEntityCard);
    case "evidence":
      return resolveLinkedItems(values, content.lookups.evidenceById, buildEvidenceCard);
    case "debates":
      return resolveLinkedItems(values, content.lookups.debatesBySlug, buildDebateCard);
    default:
      return [];
  }
}
